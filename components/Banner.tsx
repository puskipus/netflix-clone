...
function Banner({ netflixOriginals }: Props) {
  ...
  return (
    <div className="flex flex-col space-y-2 text-justify md:space-y-4 mb-20 lg:h-[65vh] lg:justify-end">
      <div className="absolute top-0 -z-10 left-0 h-[95vh] w-screen">
        <Image style={{ objectFit: "cover" }} alt={`${movie?.title || movie?.name || movie?.original_name}`} src={`${baseUrl}${movie?.backdrop_path || movie?.poster_path}`} fill />
      </div>
